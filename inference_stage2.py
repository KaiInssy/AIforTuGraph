...
# STAGE 2 ADAPTER PATH
STAGE2_DIR = "output_swift/stage2"
ADAPTER_PATH = None

if os.path.exists(STAGE2_DIR):
    # Find the latest checkpoint recursively
    print(f"Searching for checkpoints in {STAGE2_DIR}...")
    checkpoints = glob.glob(os.path.join(STAGE2_DIR, "**", "checkpoint-*"), recursive=True)
    ...
    if valid_checkpoints:
        ...
        print(f"Auto-detected Stage 2 checkpoint: {ADAPTER_PATH}")
    else:
        print(f"Warning: {STAGE2_DIR} exists but contains no valid 'checkpoint-*' folders (with adapter_config.json).")
else:
    # Use Fallback if user used old output directory name
    if os.path.exists("output_cypher_lora_all"):
        ADAPTER_PATH = "output_cypher_lora_all"
        print(f"Using fallback Stage 2 path: {ADAPTER_PATH}")

if ADAPTER_PATH is None:
    print(f"ERROR: Could not find any Stage 2 checkpoint in {STAGE2_DIR} or output_cypher_lora_all")
    # Set a dummy path so it fails gracefully later or user can edit
    ADAPTER_PATH = "output_swift/stage2/checkpoint-xxx"

TEST_FILE = "test_cypher.json"
OUTPUT_FILE = "submit_cypher_stage2.json"
# -------------------------------------------------
